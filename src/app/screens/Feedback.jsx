...
const Chip = ({ label, active, onPress }) => (
  <TouchableOpacity
    ...
    style={[
      styles.chip,
      { backgroundColor: active ? "#FFF3E9" : "white", borderColor: active ? ORANGE : "#E5E7EB" }
    ]}
  >
    <Text style={[styles.chipText, { color: active ? ORANGE : "#6B7280" }]}>{label}</Text>
  </TouchableOpacity>
)
          ...
          <View style={[styles.card, styles.visibleCard]}>
            ...
            <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 8 }}>
              <Text style={{ fontSize: 12, color: text.trim().length < MIN ? "#EF4444" : "#6B7280" }}>
                {text.trim().length < MIN ? `At least ${MIN} characters (${MIN - text.trim().length} more)` : "Looks good"}
              </Text>
...
const styles = StyleSheet.create({
  ...
  chip: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20, borderWidth: 1, marginRight: 8, marginBottom: 8 },
  chipText: { fontSize: 13, fontWeight: "600" },
  textInput: { minHeight: 120, padding: 12, borderWidth: 1, borderColor: "#E5E7EB", borderRadius: 12, color: "#0f172a", fontSize: 14, backgroundColor: "#f9fafb" },
  btnOrange: { backgroundColor: ORANGE, borderRadius: 12, paddingVertical: 14, marginTop: 16, alignItems: "center" },
