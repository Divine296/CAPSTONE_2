  ...
  return (
    <View style={styles.inputWrapper}>
      ...
      <TouchableOpacity onPress={() => setShowPwd(!showPwd)} style={styles.eyeIcon}>
        <Ionicons name={showPwd ? "eye" : "eye-off"} size={20} color="#6b7280" />
      </TouchableOpacity>
    </View>
  )
}
...
export default function PersonalInfoScreen() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setFaceSupported(
        compatible && types.includes(LocalAuthentication.AuthenticationType.FACIAL_RECOGNITION)
      )
  ...
  const strengthColor =
    strength === "Weak" ? "#ef4444" : strength === "Medium" ? "#eab308" : "#22c55e"
  ...
  const handleFaceRecognition = async () => {
    if (!faceSupported || !biometricEnrolled) {
      Alert.alert("Not Available", "Face recognition is not supported on this device.")
      return
    }
    ...
    <SafeAreaView style={[styles.container,  { paddingTop: insets.top }]}>
      ...
      <ScrollView contentContainerStyle={{ padding: 10, paddingBottom: 40 }}>
        ...
        <FieldCard title="Change Password">
          ...
          <PasswordInput value={oldPwd} onChangeText={setOldPwd} placeholder="Old Password" />

          <Label text="New Password" />
          <PasswordInput value={newPwd} onChangeText={setNewPwd} placeholder="New Password" />
          {newPwd.length > 0 && (
            <Text style={[styles.strengthText, { color: strengthColor }]}>Strength: {strength}</Text>
          )}

          <Label text="Confirm Password" />
          <PasswordInput value={confirmPwd} onChangeText={setConfirmPwd} placeholder="Confirm Password" />

          <TouchableOpacity style={styles.btnTeal} onPress={handlePasswordChange}>
        ...
        {faceSupported && (
          <FieldCard title="Face Recognition">
            <Text style={styles.grayText}>Use Face ID to enhance account security.</Text>
            <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  btnTeal: { backgroundColor: "#0d9488", borderRadius: 12, padding: 12, marginTop: 12 },
  btnOrange: {
