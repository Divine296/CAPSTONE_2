...
export default function DietaryPreferencesScreen() {
  ...
  // Allergy preferences
  const [glutenFree, setGlutenFree] = useState(preferences.allergies?.glutenFree || false);
  const [dairyFree, setDairyFree] = useState(preferences.allergies?.dairyFree || false);
  const [nutsFree, setNutsFree] = useState(preferences.allergies?.nutsFree || false);
  const [shellfishFree, setShellfishFree] = useState(preferences.allergies?.shellfishFree || false);
  const [eggFree, setEggFree] = useState(preferences.allergies?.eggFree || false);
  const [otherAllergies, setOtherAllergies] = useState(preferences.allergies?.otherAllergies || "");

  // Diet type
  const [vegetarian, setVegetarian] = useState(preferences.dietType?.vegetarian || false);
  const [vegan, setVegan] = useState(preferences.dietType?.vegan || false);
  const [pescatarian, setPescatarian] = useState(preferences.dietType?.pescatarian || false);
  const [keto, setKeto] = useState(preferences.dietType?.keto || false);
  const [lowCarb, setLowCarb] = useState(preferences.dietType?.lowCarb || false);
  const [otherDiet, setOtherDiet] = useState(preferences.dietType?.otherDiet || "");

  // Food preferences
  const [spicy, setSpicy] = useState(preferences.foodPreferences?.spicy || false);
  const [sweet, setSweet] = useState(preferences.foodPreferences?.sweet || false);
  const [salty, setSalty] = useState(preferences.foodPreferences?.salty || false);
  const [otherFoodPref, setOtherFoodPref] = useState(preferences.foodPreferences?.otherFoodPref || "");

  const savePreferences = () => {
    setPreferences({
      allergies: { glutenFree, dairyFree, nutsFree, shellfishFree, eggFree, otherAllergies },
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40, paddingTop: 16 }}>
        ...
        <Section title="Allergies">
          <Row title="Gluten" value={glutenFree} onValueChange={setGlutenFree} />
          <Row title="Dairy" value={dairyFree} onValueChange={setDairyFree} />
          <Row title="Nuts" value={nutsFree} onValueChange={setNutsFree} />
          <Row title="Shellfish" value={shellfishFree} onValueChange={setShellfishFree} />
        ...
        <Section title="Diet Type">
          <Row title="Vegetarian" value={vegetarian} onValueChange={setVegetarian} />
          <Row title="Vegan" value={vegan} onValueChange={setVegan} />
          <Row title="Pescatarian" value={pescatarian} onValueChange={setPescatarian} />
